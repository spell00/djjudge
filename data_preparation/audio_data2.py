...
class WavenetDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if valid:
            self.dataset_file = dataset_file + "_valid"
            self.scores_file = dataset_file + "_scores_valid"
        else:
            self.dataset_file = dataset_file + "_train"
            self.scores_file = dataset_file + "_scores_train"
        self._item_length = item_length
        self._test_stride = test_stride
        self.target_length = target_length
        self.classes = classes

        if not os.path.isfile(self.dataset_file + ".npz"):
            assert file_location is not None and scores_file_location is not None, "no location for dataset files specified"
            self.mono = mono
            self.normalize = normalize

            self.sampling_rate = sampling_rate
            self.dtype = dtype
            self.create_dataset(file_location, scores_file_location, train_ratio=0.8, valid=valid)
        else:
        ...
        self.data = np.load(self.dataset_file + ".npz", mmap_mode='r')
    ...
    def create_dataset(self, location, scores_file_location, train_ratio, valid=False):
        print("create dataset from audio files at", location)

        files = list_all_audio_files(location)
        processed_files = []
        try:
            scores = pd.read_csv(scores_file_location, header=None)[0].to_numpy()
        except:
            scores = pd.read_csv(scores_file_location, header=None, sep="\t")[0].to_numpy()
        files2 = []
        scores2 = []
        if not valid:
        ...
        np.savez(self.scores_file, *self.scores)
