...
def dilate(x, dilation, init_dilation=1, pad_start=True):
    ...
    if new_l != l:
        l = new_l
        x = constant_pad_1d(input=x, target_size=new_l, dimension=2, pad_start=pad_start)
...
class DilatedQueue:
    def __init__(self, max_length, data=None, dilation=1, num_deq=1, num_channels=1, dtype=torch.FloatTensor):
        ...
        self.dtype = dtype
        if data == None:
            self.data = dtype(num_channels, max_length).zero_().clone().detach().requires_grad_(True)
            # self.data._requires_grad()
...
class ConstantPad1d_new(Function):
    @staticmethod
